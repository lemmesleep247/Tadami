...
class AchievementEventBus {
     ...
     *
     * Параметры:
     * - replay = 1: новый подписчик (рестарт handler'а) репроцессит ПОСЛЕДНЕЕ событие; правила
     *   пересчитывают прогресс из БД, поэтому репроцессинг идемпотентен (E-L: KDoc ранее заявлял
     *   replay = 0, противореча коду)
