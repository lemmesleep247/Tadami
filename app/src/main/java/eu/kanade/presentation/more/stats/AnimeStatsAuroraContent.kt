                    ...
                    progressBars = listOf(
                        remember(state.episodes.readEpisodeCount, state.episodes.totalEpisodeCount, accentColor) {
                            StatsAuroraProgressData(
                                fraction = if (state.episodes.totalEpisodeCount > 0) {
                                    state.episodes.readEpisodeCount.toFloat() / state.episodes.totalEpisodeCount
                                } else {
                                    0f
                                },
                                color = accentColor,
                        ...
                        remember(state.episodes.downloadCount, state.episodes.totalEpisodeCount) {
                            StatsAuroraProgressData(
                                fraction = if (state.episodes.totalEpisodeCount > 0) {
                                    state.episodes.downloadCount.toFloat() / state.episodes.totalEpisodeCount
                                } else {
                                    0f
                                },
